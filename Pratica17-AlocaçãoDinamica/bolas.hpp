...
int questao3() {
	...
	/* Uma sequencia de muito código e pouco reuso.
	 * Use uma função pra fazer isso!
	 ...
	 g = rand() % 256; // gera um numero entre 0 e 255
	 b = rand() % 256; // gera um numero entre 0 e 255
	 alpha = 200+ rand() % 56; // gera um numero entre 150 e 255

	 sf::Color cor(r, g, b, alpha); //https://www.sfml-dev.org/documentation/2.6.1/classsf_1_1Color.php
	 Bola bola1(x, y, vx, vy, cor, raio);
	 */

	//Bem mais Facil criar uma bolinha!
	Bola* bolas = new Bola[10];
	for (int i = 0; i < 10; ++i) {
		bolas[i] = Bola::criaBolinha();
	}
