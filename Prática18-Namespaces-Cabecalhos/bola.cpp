...
namespace lp1{
//Construtor sem parametros que
//Cria uma bolinha com valores padrão
Bola::Bola() {
	x = y = vx = vy = 0;
	this->raio = 30;
	cor = sf::Color::White;
...
Bola::Bola(int x, int y, int vx, int vy, sf::Color cor, int raio) {
	this->x = x;
	this->y = y;
	this->vx = vx;
	this->vy = vy;
...
void Bola::testaColisaoCircular(Bola *outra) {
	int distanciaAtual = sqrt(
			(outra->x - x * outra->x - x) + (outra->y - y * outra->y - y));
...
void Bola::setColor(sf::Color novaCor) {
	this->cor = novaCor;
